...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/07/27 19:07:40 by vdaemoni          #+#    #+#             */
/*   Updated: 2020/07/27 19:07:40 by vdaemoni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

static void	positive(t_fdf *data)
{
	int err;
	int i;
...
static void	negative(t_fdf *data)
{
	int err;
	int i;
...
void		draw_all_y(t_fdf *data)
{
	...
	while (i < data->south - 1)
	{
		while (data->turn < data->east[i])
		{
			...
			data->y1 = data->view_y + (data->turn + i) * data->linesizey
				- (data->tab[i][data->turn] * data->z);
			data->x2 = data->view_x + ((data->turn) - (i + 1)) * data->linesizex;
			data->y2 = data->view_y + ((data->turn) + (i + 1)) * data->linesizey
